    ...
    unsigned char* d_flags)     // size <- numEdges
{
    ...
    if(tid == 0) {
        printf("delete_batch_size: %d and num of edges: %lu\n", delete_batch_size, num_edges);
    }
...
void update_edgelist_bfs(
    ...
    sort_array_uint64_t_(d_edge_list, num_edges);
    ...
    #ifdef CHECKER
        std::cout << "Checker is active in cudaBFS, in update_edgelist_bfs func.\n";
        ...
        std::cout << "Is sorted from cuda_BFS: " << std::is_sorted(h_edge_list, h_edge_list + num_edges) << std::endl;
    ...
    std::cout << "\nnumEdges after deleting batch B of edges: " << num_edges << "\n";
...
void serial_deletion(
    std::vector<uint64_t>& input_edge_list, 
    const std::vector<uint64_t>& edges_to_delete,
    std::vector<uint64_t>& output_edge_list) {

    std::cout << "Executing serial_deletion:\n";
    // It's important that output_edge_list is cleared to ensure it only contains the result of this operation
    output_edge_list.clear();

    // We use remove_if to move the elements we want to keep to the front of input_edge_list
    auto it = std::remove_if(input_edge_list.begin(), input_edge_list.end(),
        [&edges_to_delete](uint64_t element) {
            return std::find(edges_to_delete.begin(), edges_to_delete.end(), element) != edges_to_delete.end();
        });

    // Move the elements that are not to be deleted to output_edge_list
    output_edge_list.insert(output_edge_list.end(), input_edge_list.begin(), it);

    // Erase the unwanted elements from input_edge_list, adjusting its size
    input_edge_list.erase(it, input_edge_list.end());
}
...
void cuda_BFS(graph& G, const std::string& delete_filename) {
    ...
    // #ifdef CHECKER
    //     std::cout << "Checker is active in cudaBFS, after update_edgelist_bfs\n";
    //     std::vector<uint64_t> output_vect;
    //     serial_deletion(G.edge_list, edges_to_delete, output_vect);
    //     // output_vect is serial output results
    //     uint64_t *h_updated_edge_list = new uint64_t[numEdges];
    //     CUDA_CHECK(cudaMemcpy(h_updated_edge_list, d_updated_ed_list, numEdges * sizeof(uint64_t), cudaMemcpyDeviceToHost),
    //         "Failed to copy back edges");
        
    //     bool err_flag = false;

    //     // sort both the h_updated_edge_list and output_vect
    //     std::sort(h_updated_edge_list, h_updated_edge_list + numEdges);
    //     std::sort(output_vect.begin(), output_vect.end());

    //     for(long i = 0; i < numEdges; ++i) {
    //         if(h_updated_edge_list[i] != output_vect[i]) {
    //             err_flag = true;
    //             std::cerr << "Error in updated edgelist array\n";
    //         }
    //     }
    //     if(!err_flag)
    //         std::cout << "Updated edgelist is correct.\n";

    //     delete[] h_updated_edge_list;
