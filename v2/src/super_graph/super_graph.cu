...
__global__
void gpu_hashtable_lookup(keyValues* hashtable, int* d_parent, int* d_val1, int* d_val2, unsigned int size)
{
...
void get_replacement_edges(dynamic_tree_manager& tree_ds, PR_RST& pr_resource_mag, const int& unique_rep_count) {
    ...
    // combine to single edges again
    combine_edges<<<numBlocks, numThreads>>>(
        d_super_graph_u,
        d_super_graph_v,
        d_super_graph,
        num_edges
    ...
    RootedSpanningTree(d_edge_list + 1, unique_super_graph_edges - 1, pr_resource_mag);
