...
std::vector<int> RootedSpanningTree(const std::vector<int>& u_arr, const std::vector<int>& v_arr, const int n) {
	...
	thrust::device_vector<int> d_u_arr(u_arr.begin(), u_arr.end());
	thrust::device_vector<int> d_v_arr(v_arr.begin(), v_arr.end());
	...
	std::cout<<"inside function received uArr and vArr as "<<std::endl;
	for(auto i : u_arr)
		std::cout << i<<" ";
