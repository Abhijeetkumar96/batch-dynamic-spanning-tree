...
log_dir="/home/graphwork/cs22s501/spanning_tree/batch-dynamic-spanning-tree/main_variants/Behaviourial_Analysis/euler_runner_log_files"
err_dir="/home/graphwork/cs22s501/spanning_tree/batch-dynamic-spanning-tree/main_variants/Behaviourial_Analysis/euler_runner_err_files"
    ...
    if [[ " ${files_to_process_str} " == *" ${filename} "* ]]; then
        ...
        if [ -d "$target_dir" ]; then
            ...
            find "$target_dir" -type f -name '*.txt' -print | while read txt_file; do
                ...
                echo -e "\nRunning command: build/dynamic_spanning_tree -i $file -b $txt_file -d 2 -r SG -p ET -s >> $file_out 2> $error_file"
                # Uncomment the following line to actually run the command
                timeout $TIMEOUT_DURATION build/dynamic_spanning_tree -i "$file" -b "$txt_file" -d 2 -r SG -p ET -s >> "$file_out" 2>> "$error_file"
