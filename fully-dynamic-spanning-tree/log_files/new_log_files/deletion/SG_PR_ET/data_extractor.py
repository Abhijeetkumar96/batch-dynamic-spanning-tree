    ...
    with open(filepath, 'r') as file:
        for line in file:
            ...
            if line.startswith("filename:"):
                ...
                current_record['tree_edges_count'] = int(re.search(r"Number of deleted tree edges: (\d+)", line).group(1))
            ...
            elif "Total execution time of all functions:" in line:
                current_record['total_time'] = float(re.search(r"Total execution time of all functions: ([\d.]+) ms", line).group(1))

            elif line.startswith("Time of"):
                function_name = re.search(r"Time of (.+?) :", line).group(1)
                function_time = float(re.search(r": ([\d.]+) ms", line).group(1))
