...
log_dir="/home/graphwork/cs22s501/spanning_tree/batch-dynamic-spanning-tree/main_variants/Behaviourial_Analysis/HS_insertion_log_files"
err_dir="/home/graphwork/cs22s501/spanning_tree/batch-dynamic-spanning-tree/main_variants/Behaviourial_Analysis/HS_insertion_err_files"
    ...
    if [[ " ${files_to_process_str} " == *" ${filename} "* ]]; then
        ...
        if [ -d "$target_dir" ]; then
            ...
            find "$target_dir" -type f -name '*.txt' -print | while read txt_file; do
                ...
                echo -e "\nRunning command: build/dynamic_spanning_tree -i $file -b $txt_file -d 1 -r HS -p PR >> $file_out 2> $error_file"
                # Run the command with a timeout
                timeout $TIMEOUT_DURATION build/dynamic_spanning_tree -i "$file" -b "$txt_file" -d 1 -r HS -p PR >> "$file_out" 2>> "$error_file"
